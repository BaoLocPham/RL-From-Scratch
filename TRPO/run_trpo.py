...
is what each does with a batch:
  vanilla PG, 1 epoch     one update per batch, with L^PG         (flaw 1: slow)
  vanilla PG, 50 epochs   50 updates per batch, with L^PG, no fence (flaw 2: overshoot)
  TRPO                    up to 50 updates per batch, with eq. 3, stopped by the KL fence
...
MAX_KL = 0.01           # delta: the most one batch may change the policy's behaviour
LR = 0.3                # the same step size for every run
EPOCHS = 50             # the most updates any run may take on one batch
SEEDS = 20


def trpo_fresh_batch_each_update(seed, iterations, n=16):
    """TRPO as intended: collect 16 NEW rollouts, run epochs on them until the fence, repeat.

    Returns (iteration, probabilities, true J) after every iteration, starting at
    0, and how many epochs each batch got.
    """
...
def vpg_many_epochs_per_batch(seed, iterations, n=16):
    """VPG's shortcut, every iteration: 50 epochs of L^PG on each fresh batch, no fence.
    ...
    for iteration in range(1, iterations + 1):
        ...
        vpg_update(policy, optimizer, batch, updates=EPOCHS)    # 50 updates on the same batch
        rows.append((iteration, policy.probs(), policy.true_reward()))
    return rows
...
def print_terms():
  ...
  delta         the most KL one batch may use. Here 0.01.
...
if __name__ == "__main__":
    ...
    print("  A = vanilla PG: 1 epoch per batch, with L^PG")
    print(f"  B = TRPO:       up to {EPOCHS} epochs on eq. 3, stopped when the KL from theta_old would pass {MAX_KL}")
    ...
    shortcut_j = reuse_one_batch(seed=17, updates=100)[-1][2]   # the VPG demo's 100 epochs, no fence
    print(f"""
Reading it:
  iteration 1   both read the same unlucky batch (it says search hurts HARD questions).
                TRPO takes {epochs[0]} epochs on it before the fence stops it, so it steps
                further the wrong way: J {vpg[1][2]:.3f} (A) vs {trpo[1][2]:.3f} (B). The fence caps that
                damage -- VPG's shortcut of 100 epochs with no fence fell to {shortcut_j:.3f}.
  ...
  Epochs per batch over the first 800 rollouts:  vanilla PG 1,  TRPO {sum(epochs[:50]) / 50:.1f} on average.
    ...
    runs = {
        "VPG, 1 epoch per batch": ([fresh_batch_each_update(seed, marks[-1]) for seed in range(SEEDS)],
                                   "slow"),
        f"VPG, {EPOCHS} epochs, no fence": ([vpg_many_epochs_per_batch(seed, marks[-1]) for seed in range(SEEDS)],
                                     "overshoots"),
        f"TRPO, <={EPOCHS} epochs, fence": ([trpo_fresh_batch_each_update(seed, marks[-1])[0] for seed in range(SEEDS)],
                                     "fast and safe"),
    }
    print(f"""
{"=" * 86}
...
{"=" * 86}""")
    print(f"  {'':<27} {'J@160':>6} {'J@400':>6} {'J@800':>6} {'stuck':>7}")
    for name, (rs, verdict) in runs.items():
        ...
        print(f"  {name:<27} " + " ".join(f"{j:>6.3f}" for j in mean_j) + f" {stuck:>4}/{SEEDS}   {verdict}")
    print("""
  vs VPG, 1 epoch      TRPO learns more from every batch (many safe epochs instead of
                       one), so it is ahead at every budget: it fixes flaw 1, slowness.
  vs VPG, 50 epochs    reusing each batch with no fence is quick at first, then locks
                       misleading batches in and gets stuck. TRPO takes up to the same
                       50 epochs, but the fence stops each batch early: it fixes flaw 2,
                       overshoot, and ends highest with no run stuck.
  stuck  = ended with p(tool|HARD) < 0.05 or p(tool|EASY) > 0.95: locked onto a wrong action.""")

    print("""
TRPO's flaw: the fence is not part of the loss. After every epoch, something outside
the loss has to measure the KL and undo the step if it went too far -- "subject to"
is not something you can call .backward() on. PPO moves the fence INSIDE the loss by
