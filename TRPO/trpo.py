...
What it fixes in VPG:
  ...
  - eq. 4 (the KL constraint) keeps every update inside the "trust region" around
...
take small gradient steps on eq. 3, reusing the same batch (several epochs over
it, like VPG's shortcut), and stop before the KL from theta_old goes past delta.

Its flaw: the KL constraint is not part of the loss. After every step something outside
...
best step inside the trust region with heavier second-order math instead; that
heaviness is what the paper calls TRPO's flaw.) PPO builds the limit INTO the
loss, with a clip.
"""
...
def trpo_update(policy, batch, max_kl=0.01, lr=0.3, epochs=50):
    ...
    each step it checks the KL constraint: if the KL from theta_old is now above
    max_kl, that step is undone and the update ends. Returns how many steps
    (epochs) were kept -- 0 if even the first step would leave the trust region, in
    which case this batch changes nothing.

    max_kl   delta in eq. 4: the most the policy's behaviour may change per iteration
    lr       the size of each small step (the same lr as VPG, for a fair comparison)
    epochs   an upper limit. Early in training the KL constraint stops the loop first;
             near the best policy the gradient is tiny, so the steps are too and
             the limit is reached instead.
    """
    qtype = batch[0]
    old_probs = policy.probs()[qtype]           # pi_theta_old(. | s_t), frozen: the centre of the trust region
    optimizer = torch.optim.SGD(policy.parameters(), lr=lr)
    kept = 0
    for _ in range(epochs):
        before = policy.logits.detach().clone()   # where we were, in case this step leaves the trust region
        ...
        # The KL constraint (eq. 4). It is not in the loss, so it has to be checked here.
